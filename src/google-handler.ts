...
import { fetchUpstreamAuthToken, getUpstreamAuthorizeUrl, Props } from './oauth-utils'
...
app.get('/authorize', async (c) => {
  ...
  return renderApprovalDialog(c.req.raw, {
    ...
    server: {
      ...
      description: 'This MCP Server allows you to generate images using AI.',
    },
    state: { oauthReqInfo },
  })
})
...
app.get('/callback', async (c) => {
  ...
  if (c.env.ALLOWED_EMAILS) {
    ...
    if (!allowedEmails.includes(email.toLowerCase())) {
      return c.text(`Access denied. Your email (${email}) is not authorized to use this service.`, 403)
    }
  }
